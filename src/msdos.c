...
mouse_get_pos (FRAME_PTR *f, int insist, Lisp_Object *bar_window,
	       Lisp_Object *x, Lisp_Object *y, enum scroll_bar_part *part,
	       unsigned long *time)
{
  int ix, iy;
  union REGS regs;
...
static void
IT_set_menu_bar_lines (Lisp_Object window, int n)
{
  struct window *w = XWINDOW (window);

  XSETINT (w->last_modified, 0);
  XSETINT (w->top, XINT (w->top) + n);
  XSETINT (w->height, XINT (w->height) - n);
...
void
IT_set_frame_parameters (FRAME_PTR frame, Lisp_Object alist)
{
  Lisp_Object tail;
  int redraw;
  ...
  if (redraw)
    {
      ...
      Fredraw_frame (Fselected_frame (Qnil), Qnil);
    }
}
...
void
glyph_to_pixel_coords (FRAME_PTR f, REGISTER int x, REGISTER int y,
		       REGISTER int *pix_x, REGISTER int *pix_y)
{
  *pix_x = x;
  *pix_y = y;
}
...
int
run_msdos_command (unsigned char **argv, Lisp_Object dir,
		   int tempin, int tempout)
{
...
vars_of_msdos (void)
{
  recent_doskeys = Fmake_vector (make_int (NUM_RECENT_DOSKEYS), Qnil);
