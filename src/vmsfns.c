  ...
  while (ptr)
      ...
      if (ptr->name == XINT (name))
	{
	  ...
	  if (! NILP (ptr->exit_handler))
	    Feval (Fcons (ptr->exit_handler, Fcons (make_int (ptr->name),
						    Qnil)));
...
static int
translate_id (Lisp_Object pid, int owner)
     ...
     Lisp_Object arg1, arg2;
{
  ...
  for (;;)
	...
	error ("Unable to get process ID: %s", vmserrstr (status));
      retval = Fcons (make_int (id), retval);
    }
