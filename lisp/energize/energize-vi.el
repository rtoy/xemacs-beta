...
(defun ex-quit (discard)
  ...
  ;; buffer and deletes the current window (and frame if appropriate.)
  ...
  (condition-case nil
    ...
    ;; ignore error about trying to delete only window on only frame
    (error nil))
  )
...
(defun energize-external-editor-set-menubar ()
  ...
  (delete-menu-item '("File" "Exit XEmacs")))
...
(defun energize-internal-editor-set-menubar ()
  ...
  (add-menu-item '("File") "Exit XEmacs" 'save-buffers-kill-emacs t))
    ...
    (let* ((type (energize-buffer-type buffer))
      ...
      (cond (editable-p
	    ...
	    ((eq energize-internal-viewer 'less)
	     ...
	     ;; so that the buffers go in the appropriate frames.
...
(defun energize-search-internal (args)
  ;; called from editorside.c
  (let* ((case-fold-search (not (nth 0 args)))
	 (match-word-p (nth 1 args))
	 (regexp-string-p (nth 2 args))	; string for which to search is regex
	 (regexp-p (or match-word-p regexp-string-p)) ; use regex function?
	 (back-p (not (nth 3 args)))
	 (search-string (nth 4 args))
	 (search (if match-word-p
		     (concat "\\b"
			     (if regexp-string-p ; coerce to regexp
				 search-string
...
windows (\"frames\" in emacs terminology) or should use and reuse one only
...
in the debugger frame, `splitting' the frame vertically, and the other is to
use two frames, one for the debugger and one for the source.  The default for
