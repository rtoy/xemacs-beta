...
;; numbers from packages, and he did in this case.
...
(defun desktop-clear () "Empty the Desktop."
  ...
  (mapcar (function (lambda (x)
		      ;; XEmacs change
		      (if (not (equal (buffer-name x) "*Warnings*"))
			  (kill-buffer x)))) (buffer-list))
...
(defun desktop-buffer-info () "Load an info file."
  (if (eq 'Info-mode desktop-buffer-major-mode)
      (progn
	(require 'info)
	(Info-find-node (nth 0 desktop-buffer-misc)
...
;;		(setq mark-active (car (cdr mk))))
                (if (car (cdr mk)) (zmacs-activate-region))) ; XEmacs
	    (set-mark mk))
