...
won't destroy Emerge control variables."
  (` (progn
       ...
       (put '(, var)
	    ...
	    ;; emerge-xemacs-p is not defined at that time.
	    (if emerge-xemacs-p 'permanent-local 'preserved)
	    t))))
...
(defvar emerge-split-horizontally t
  ;; XEmacs - even when this works, jwz hates it.
  "*Flag to control whether emerge will split windows horizontally or not.
Nil means that windows will be split vertically.") 
...
(defvar emerge-exit-func)		; was just exit-func, but fsf
					; renamed it and it's not used anywhere
...
;;fsf   (define-key emerge-basic-keymap [menu-bar] (make-sparse-keymap))
;;fsf 
;;fsf   (define-key emerge-fast-keymap [menu-bar options]
;;fsf     (cons "Options" emerge-options-menu))
;;fsf   (define-key emerge-fast-keymap [menu-bar merge]
;;fsf     (cons "Merge" emerge-merge-menu))
;;fsf   (define-key emerge-fast-keymap [menu-bar move]
;;fsf     (cons "Move" emerge-move-menu))
;;fsf 
;;fsf   (define-key emerge-move-menu [emerge-scroll-reset]
;;fsf     '("Scroll Reset" . emerge-scroll-reset))
;;fsf   (define-key emerge-move-menu [emerge-scroll-right]
;;fsf     '("Scroll Right" . emerge-scroll-right))
;;fsf   (define-key emerge-move-menu [emerge-scroll-left]
;;fsf     '("Scroll Left" . emerge-scroll-left))
;;fsf   (define-key emerge-move-menu [emerge-scroll-down]
;;fsf     '("Scroll Down" . emerge-scroll-down))
;;fsf   (define-key emerge-move-menu [emerge-scroll-up]
;;fsf     '("Scroll Up" . emerge-scroll-up))
;;fsf   (define-key emerge-move-menu [emerge-recenter]
;;fsf     '("Recenter" . emerge-recenter))
;;fsf   (define-key emerge-move-menu [emerge-mark-difference]
;;fsf     '("Mark Difference" . emerge-mark-difference))
;;fsf   (define-key emerge-move-menu [emerge-jump-to-difference]
;;fsf     '("Jump To Difference" . emerge-jump-to-difference))
;;fsf   (define-key emerge-move-menu [emerge-find-difference]
;;fsf     '("Find Difference" . emerge-find-difference))
;;fsf   (define-key emerge-move-menu [emerge-previous-difference]
;;fsf     '("Previous Difference" . emerge-previous-difference))
;;fsf   (define-key emerge-move-menu [emerge-next-difference]
;;fsf     '("Next Difference" . emerge-next-difference))
;;fsf 
;;fsf 
;;fsf   (define-key emerge-options-menu [emerge-one-line-window]
;;fsf     '("One Line Window" . emerge-one-line-window))
;;fsf   (define-key emerge-options-menu [emerge-set-merge-mode]
;;fsf     '("Set Merge Mode" . emerge-set-merge-mode))
;;fsf   (define-key emerge-options-menu [emerge-set-combine-template]
;;fsf     '("Set Combine Template..." . emerge-set-combine-template))
;;fsf   (define-key emerge-options-menu [emerge-default-B]
;;fsf     '("Default B" . emerge-default-B))
;;fsf   (define-key emerge-options-menu [emerge-default-A]
;;fsf     '("Default A" . emerge-default-A))
;;fsf   (define-key emerge-options-menu [emerge-skip-prefers]
;;fsf     '("Skip Prefers" . emerge-skip-prefers))
;;fsf   (define-key emerge-options-menu [emerge-auto-advance]
;;fsf     '("Auto Advance" . emerge-auto-advance))
;;fsf   (define-key emerge-options-menu [emerge-edit-mode]
;;fsf     '("Edit Mode" . emerge-edit-mode))
;;fsf   (define-key emerge-options-menu [emerge-fast-mode]
;;fsf     '("Fast Mode" . emerge-fast-mode))
;;fsf 
;;fsf   (define-key emerge-merge-menu [emerge-abort] '("Abort" . emerge-abort))
;;fsf   (define-key emerge-merge-menu [emerge-quit] '("Quit" . emerge-quit))
;;fsf   (define-key emerge-merge-menu [emerge-split-difference]
;;fsf     '("Split Difference" . emerge-split-difference))
;;fsf   (define-key emerge-merge-menu [emerge-join-differences]
;;fsf     '("Join Differences" . emerge-join-differences))
;;fsf   (define-key emerge-merge-menu [emerge-trim-difference]
;;fsf     '("Trim Difference" . emerge-trim-difference))
;;fsf   (define-key emerge-merge-menu [emerge-combine-versions]
;;fsf     '("Combine Versions" . emerge-combine-versions))
;;fsf   (define-key emerge-merge-menu [emerge-copy-as-kill-B]
;;fsf     '("Copy B as Kill" . emerge-copy-as-kill-B))
;;fsf   (define-key emerge-merge-menu [emerge-copy-as-kill-A]
;;fsf     '("Copy A as Kill" . emerge-copy-as-kill-A))
;;fsf   (define-key emerge-merge-menu [emerge-insert-B]
;;fsf     '("Insert B" . emerge-insert-B))
;;fsf   (define-key emerge-merge-menu [emerge-insert-A]
;;fsf     '("Insert A" . emerge-insert-A))
;;fsf   (define-key emerge-merge-menu [emerge-select-B]
;;fsf     '("Select B" . emerge-select-B))
;;fsf   (define-key emerge-merge-menu [emerge-select-A]
;;fsf     '("Select A" . emerge-select-A)))

(defvar emerge-menu
  '(["Buffers ..."			emerge-buffers			t]
    ["Buffers With Ancestor ..."	emerge-buffers-with-ancestor	t]
    ["Files ..."			emerge-files			t]
    ["Files with Ancestor ..."		emerge-files-with-ancestor	t]
    ["Revisions ..."			emerge-revisions		t]
    ["Revisions with Ancestor ..."	emerge-revisions-with-ancestor	t]
    ["Merge Directories ..."		emerge-merge-directories	t])
  "Menubar entry for using Emerge.")

(defvar emerge-mode-menu
  '(
    ["Next Difference"			emerge-next-difference		t]
    ["Previous Difference"		emerge-previous-difference	t]
    ("Other Motion"
     ["Find Difference"			emerge-find-difference		t]
     ["Jump To Difference"		emerge-jump-to-difference	t]
     ["Mark Difference"			emerge-mark-difference		t]
     ["Recenter"			emerge-recenter			t]
     ["Scroll Up"			emerge-scroll-up		t]
     ["Scroll Down"			emerge-scroll-down		t]
     ["Scroll Left"			emerge-scroll-left		t]
     ["Scroll Right"			emerge-scroll-right		t]
     ["Scroll Reset"			emerge-scroll-reset		t])
    "---"
    ["Select A"				emerge-select-A			t]
    ["Select B"				emerge-select-B			t]
    ["Insert A"				emerge-insert-A			t]
    ["Insert B"				emerge-insert-B			t]
    ["Copy A as Kill"			emerge-copy-as-kill-A		t]
    ["Copy B as Kill"			emerge-copy-as-kill-B		t]
    ("Other Merge"
      ["Combine Versions"		emerge-combine-versions		t]
      ["Trim Difference"		emerge-trim-difference		t]
      ["Join Differences"		emerge-join-differences		t]
      ["Split Difference"		emerge-split-difference		t])
    "---"
    ("Options"
     ["Fast Mode"			emerge-fast-mode		t]
     ["Edit Mode"			emerge-edit-mode		t]
     ["Auto Advance"			emerge-auto-advance		t]
     ["Skip Prefers"			emerge-skip-prefers		t]
     ["Default A"			emerge-default-A		t]
     ["Default B"			emerge-default-B		t]
     ["Set Combine Template..."		emerge-set-combine-template	t]
     ["Set Merge Mode"			emerge-set-merge-mode		t]
     ["One Line Window"			emerge-one-line-window		t])
    ["Abort"				emerge-abort			t]
    ["Quit"				emerge-quit			t])
  "Menubar entry while in Emerge minor mode.")
...
(defun emerge-set-keys ()
  ...
  (emerge-force-define-key emerge-edit-keymap emerge-command-prefix
			   'emerge-basic-keymap)
  ;;fsf   (define-key emerge-edit-keymap [menu-bar] (make-sparse-keymap))
  ;;fsf 
  ;;fsf   ;; Create the additional menu bar items.
  ;;fsf   (define-key emerge-edit-keymap [menu-bar options]
  ;;fsf     (cons "Options" emerge-options-menu))
  ;;fsf   (define-key emerge-edit-keymap [menu-bar merge]
  ;;fsf     (cons "Merge" emerge-merge-menu))
  ;;fsf   (define-key emerge-edit-keymap [menu-bar move]
  ;;fsf     (cons "Move" emerge-move-menu))
  ...
  (add-menu nil "Emerge" emerge-mode-menu)
...
(defun emerge-really-quit (arg)
  ...
  ;; restore the menubar
  ;; don't make the menubar become un-local because the major mode might
  ;; have already made it local and added an item.
  (delete-menu-item '("Emerge"))
...
(defun emerge-place-flags-in-buffer1 (difference before-index after-index)
  (if (and emerge-xemacs-p (not emerge-mark-with-text))
      ;; XEmacs highlights the difference
      (emerge-place-flags-in-buffer1-xemacs difference before-index after-index)
...
(defun emerge-place-flags-in-buffer1-xemacs (difference before-index
						       after-index)
  (let* ((before (aref (aref emerge-globalized-difference-list difference)
		       before-index))
...
(defun emerge-read-file-name (prompt alternative-default-dir default-file
  ...
  ;; 'must-match' controlls whether non-existent file names are allowed;
...
;; XEmacs -- nuked a bunch of junk here that provides a newer version
;; of `describe-mode' that describes minor modes as well as the major
;; mode.  We have this code standardly.
