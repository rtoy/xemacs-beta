...
;; Created: 14 Jul 1992
;; Version: $Id: lisp-mnt.el,v 1.1.1.1 1996/12/18 03:31:04 steve Exp $
;; Keywords: docs
...
(defun lm-section-mark (hd &optional after)
  ...
  (save-excursion
    (let ((case-fold-search t))
      (goto-char (point-min))
      (if (re-search-forward (concat "^;;;* " hd "[:.][ \t\n]*") nil t)
	  (if after
	      (point)
	    (match-beginning 0))))))
...
(defun lm-commentary (&optional file)
  ...
  (save-excursion
	...
	(let ((commentary (lm-section-mark "Commentary" t))
	      (end (lm-section-mark "Change Log")))
	  (if commentary
	      (if end
		  (buffer-substring commentary end)
		(setq end (lm-section-mark "Code"))
		(buffer-substring commentary end))
	    (setq commentary (lm-section-mark "DESCRIPTION" t)
		  end (lm-section-mark "DESCRIP-END"))
