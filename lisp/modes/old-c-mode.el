...
;; XEmacs change: if cc-mode is loaded, then override its keymap.
;; otherwise things will be really screwed up.
(if (or (null c-mode-map) (featurep 'cc-mode))
    (progn
...
(defvar c-mode-popup-menu
  (purecopy '("C Menu"
	      ...
	      ["Macro Expand Region"	c-macro-expand	(region-exists-p)]
	      ["Backslashify"		c-backslash-region (region-exists-p)]
	      "---"
	      ["Indent Expression"	indent-c-exp			t]
	      ["Indent Line"	c-indent-command			t]
	      ["Fill Comment Paragraph"	c-fill-paragraph		t]
	      "---"
	      ["Highlight Conditionals"	cpp-highlight-buffer		t]
...
(defvar c-mode-menubar-menu
  (purecopy (cons "C" (cdr c-mode-popup-menu))))
...
;;; statements.
(defvar c-switch-label-regexp (purecopy "case[ \t'/(]\\|default[ \t]*:"))
...
if that value is non-nil."
  ...
  (setq mode-popup-menu c-mode-popup-menu)
  (setq mode-name "C")
  ...
  (if (featurep 'menubar)
	...
	(add-submenu nil c-mode-menubar-menu)))
	 ...
	 (contain-stack
	  (list (if endpos
			...
			(condition-case nil
			  ...
			  (error
;;;#### Commented-out in XEmacs, present in FSFmacs
;                          (setq opoint funbeg)
                           (point)))))
		  (point))))
	 (case-fold-search nil)
