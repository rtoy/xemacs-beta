...
;; Modified:     Wed May  3 00:50:40 1995 (Andy Norman) ange@hplb.hpl.hp.com
;; Modified for XEmacs by jwz
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;; XEmacs patch from Bob Weiner
(defvar ange-ftp-multi-msgs
  "^331-\\|^220-\\|^230-\\|^226\\|^25.-\\|^221-\\|^200-\\|^530-\\|^4[25]1-"
...
(defvar ange-ftp-default-user "anonymous"
...
(defvar ange-ftp-generate-anonymous-password t ;; changed wing@666.com
...
(defconst ange-ftp-version "Revision: 4.20.XEmacs")
...
;; New error symbols.
;; XEmacs change
(define-error 'ftp-error "FTP error" 'file-error)

;;; ------------------------------------------------------------
...
into one of four categories: good, skip, fatal, or unknown."
  (cond ((string-match ange-ftp-xfer-size-msgs line)
	...
	((string-match ange-ftp-skip-msgs line)
 	 (setq ange-ftp-process-multi-skip nil) ;; XEmacs patch (Bob Weiner)
	 t)
...
on the gateway machine to do the ftp instead."
  (let* ((use-gateway (ange-ftp-use-gateway-p host))
	 ...
	 (saved-term-var (getenv "TERM"))
	 proc)
    ;; fix problems in losing Linux FTP's, which like to output
    ;; ESC sequences to highlight the ftp prompt, which messes things up
    (unwind-protect
	(progn
	  (setenv "TERM" "dumb")
	  (if use-gateway
...
;; XEmacs change: This function isn't in the FSF version.  Maybe
;; because there is no such function as diff-read-args.  I can't find
;; where there ever _has_ been such a function.  If you want this
;; functionality, write diff-read-args and uncomment this.
	      ...
	      (let* ((host (nth 0 parsed))
		     ...
		     (result (ange-ftp-send-cmd host user
						...
						(format "doing chmod %s"
							abbr))))
		(or (car result)
		    ;; if FTP server rejects chmod, try rsh chmod instead
		    (call-process 
		     ange-ftp-remote-shell
		     nil t nil host "chmod" mode path)))))))
     (cdr args)))
      ...
      (cond ((listp nfun)
	    ...
	    (t
		 ...
		 (let ((new-code (append nfun nil))) ; turn it into a list
	       ...
	       ;; the new way (marginally less random) for XEmacs 19.8+
	       (fset new
...
(defun ange-ftp-overwrite-dired ()
  (if (not (fboundp 'dired-ls))		;dired should have been loaded by now
    ...
    ;; XEmacs - not anymore because ange-ftp-diff is hosed
...
(if (and (fboundp 'remove-directory)
    ...
    (ange-ftp-overwrite-fn 'remove-directory))
;; XEmacs change -- ange-ftp-diff is hosed
;;(if (and (fboundp 'diff)
...
;; This is so that VC doesn't need to be hacked up.  I think the fsf way is
;; a bit cleaner.  (Forgive me, as I have sinned...)  The great side-effect 
;; of this change is that ange-ftp will now autoload...even w/o being fully
;; converted to use the filename-handler-alist.  --Stig
...
;;;###autoload
(or (assoc (car ange-ftp-path-format) file-name-handler-alist)
    (setq file-name-handler-alist
          (cons (cons (car ange-ftp-path-format) 'ange-ftp-hook-function)
